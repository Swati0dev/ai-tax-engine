...
export class XMLParserStrategy implements IParserStrategy {
  public name = 'XMLParser';

  public supports(mimeType: string): boolean {
    return mimeType === ParserMimeType.XML;
  }
  ...
  public async parse(context: ParserContext): Promise<ParserResult> {
    ...
    if (!this.validate(context)) {
      result.errors.push('Empty buffer provided to XML Parser');
      return result;
    }

    try {
      result.extractedContent = `[XML_PLACEHOLDER] Buffer size: ${context.rawBuffer.length} bytes`;
